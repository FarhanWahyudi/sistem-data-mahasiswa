...
<div class="flex flex-col gap-8 pb-10">
    <div class="flex items-center justify-between">
        <div class="text-white font-medium">
            <span class="text-sm"><a href="{{ route('dashboard.view') }}" class="text-gray-300">Admin</a> / Kota</span>
            <h1 class="font-semibold">Daftar Kota</h1>
        </div>
    ...
    <div class="flex items-start flex-wrap gap-5 2xl:gap-8">
        <div class="w-full xl:w-[60%] bg-white rounded-xl border border-gray-200 p-5 2xl:p-8 dark:bg-[#132347] dark:border-[#132347] transition-all duration-300">
            <div class="flex items-start justify-between gap-2">
                <h2 class="text-gray-700 text-lg font-medium dark:text-white">Daftar Kota</h2>
                <button class="btn-add-city hidden gap-2 text-xs bg-blue-500 px-3 py-1.5 rounded-md text-gray-50 font-medium md:block md:text-sm">
                    Tambah Kota
                </button>
                <button class="btn-add-city gap-2 text-xs bg-blue-500 px-3 py-1.5 rounded-md text-gray-50 font-medium md:text-sm md:hidden">
                    +
                </button>
            </div>
                    ...
                    <tbody>
                        ...
                        @foreach ($cities as $city)
                            <tr class="border-b border-gray-200 dark:border-gray-400 transition-all duration-300">
                                <td class="px-3 text-xs py-4 text-gray-700 dark:text-gray-300 md:text-sm">{{ $city->name }}</td>
                                <td class="px-3 text-xs py-4 text-gray-700 dark:text-gray-300 md:text-sm">{{ $city->total_students }}</td>
                                <td class="px-3 py-4">
                                    <div class="flex gap-2 md:gap-3">
                                        <button class="btn-show" data-id="{{ $city->id }}">
                                        ...
                                        <button class="btn-edit" data-id="{{ $city->id }}">
                                        ...
                                        <form method="POST" action="{{ route('city.destroy', $city->id) }}" class="flex justify-center items-center">
                                            @csrf
                                            <button type="submit">
        ...
        <div id="modal" class="hidden fixed top-0 left-0 w-full h-screen justify-center items-center xl:flex xl:flex-1 xl:static xl:w-auto xl:h-auto bg-black/50 backdrop-blur-sm xl:bg-transparent">
            <div id="modal-content" class="w-[90%] max-h-[80vh] overflow-y-auto scrollbar-hide bg-white rounded-xl border border-gray-200 p-5 2xl:p-8 dark:bg-[#132347] dark:border-[#132347]  sm:w-[80%] md:w-[60%] lg:w-[50%] xl:h-auto xl:w-full transition-all duration-300">
                ...
                {{-- empty city --}}
                <div id="data-city-empty" class="h-96 flex flex-col justify-center items-center gap-3">
                ...
                {{-- add city --}}
                <div id="add-city" class="hidden">
                    <div class="flex items-center justify-between">
                        <h2 class="text-gray-700 font-medium text-lg dark:text-white">Tambah Kota</h2>     
                        <button class="xl:hidden btn-close-city">
                    ...
                    <form id="city-form" method="POST">
                        ...
                        <div class="flex flex-col gap-4 mt-5">
                            <div class="flex flex-col flex-wrap 2xl:flex-row 2xl:items-center">
                                <label for="city" class="text-gray-600 dark:text-gray-200 text-sm sm:text-base">KOTA</label>
                                <input name="city" type="text" class="w-full bg-transparent mt-2 border border-indigo-500 rounded-md text-gray-900 dark:text-gray-200 text-sm sm:text-base" value="{{ old('city') }}">
                                <span id="name-error" class="error-city-message w-full text-red-500 mt-0.5 text-xs sm:text-sm"></span>
                            </div>
                            <div class="flex justify-end gap-2">
                                <button type="button" class="btn-close-city bg-red-500 font-medium px-3 py-1.5 text-white rounded-md text-sm sm:text-base">Batal</button>
                ...
                {{-- show city --}}
                <div id="show-city" class="hidden">
                    <div class="flex items-start justify-between">
                        <div>
                            <h2 class="text-gray-700 font-medium text-lg dark:text-white">Daftar Mahasiswa</h2>     
                            <span id="city-label" class="text-gray-600 dark:text-gray-300"></span>
                        </div>
                        <button class="mt-2 xl:hidden btn-close-city">
                            <svg class="text-red-500 w-2.5" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path d="M376.6 84.5c11.3-13.6 9.5-33.8-4.1-45.1s-33.8-9.5-45.1 4.1L192 206 56.6 43.5C45.3 29.9 25.1 28.1 11.5 39.4S-3.9 70.9 7.4 84.5L150.3 256 7.4 427.5c-11.3 13.6-9.5 33.8 4.1 45.1s33.8 9.5 45.1-4.1L192 306 327.4 468.5c11.3 13.6 31.5 15.4 45.1 4.1s15.4-31.5 4.1-45.1L233.7 256 376.6 84.5z"/></svg>
                        </button>
                    </div>
                    <div class="overflow-x-auto mt-5">
                ...
                <div id="edit-city" class="hidden">
                    ...
                    <form id="city-edit-form" method="POST">
                        @csrf
                        <input type="hidden" name="id-city" id="input-id">
                        <div class="flex flex-col gap-4 mt-5">
                            <div class="flex flex-wrap flex-col gap-2 2xl:flex-row 2xl:gap-2 2xl:items-center">
                                ...
                                <input id="input-name" name="city" type="text" class="w-full bg-transparent border border-indigo-500 rounded-md text-gray-900 dark:text-gray-200 text-sm sm:text-base">
                                <span id="edit-name-error" class="error-city-message w-full text-red-500 mt-0.5 text-xs sm:text-sm"></span>
                            </div>
                            <div class="flex justify-end gap-2">
                                <button type="button" class="btn-close-city bg-red-500 font-medium px-3 py-1.5 text-white rounded-md text-sm sm:text-base">Batal</button>
                                <button type="submit" class="bg-blue-500 font-medium px-3 py-1.5 text-white rounded-md text-sm sm:text-base">Simpan</button>
...
@push('scripts')
    @vite('resources/js/cityAction.js')
@endpush